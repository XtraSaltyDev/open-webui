...
- `AppSettings.embeddingModelID` can pin knowledge import and retrieval to a dedicated embedding model. When unset, `AppStore.embeddingModelCandidates` prefers active-provider models whose id/name/details look embedding-specific, then falls back to unlabeled provider models for custom gateways before using the active chat model as a last resort.
...
- `KnowledgeDocumentDetailView` previews the indexed chunk text for the selected document so users can inspect the RAG source material.
...
The production version should replace or wrap the JSON snapshot with SQLite or a dedicated local vector backend, add richer document parser services and collection/document metadata editing beyond names, add answer-span citation mapping, and filter embedding settings to embedding-capable models where providers expose that metadata.
...
Future work should add Python execution isolation, tool-call routing into provider requests, nested/generated advanced JSON Schema forms, MCP SSE/session hardening, long-running stdio lifecycle management, and richer audit logs.
...
- `CalendarRecurrenceService` expands daily, weekly, monthly, and simple yearly `rrule` values, including `INTERVAL`, `COUNT`, `UNTIL`, weekly `BYDAY`, and monthly `BYMONTHDAY`, into shifted event occurrences for visible date ranges; unsupported rules still display the original stored event if it overlaps the range.
...
Future work should add advanced RRULE support beyond `COUNT`/`UNTIL`, richer shared-calendar workflows, closed-app reminder guarantees, server-backed permission enforcement, and richer saved-search/filter workflows.
...
Future work should add formal provider-reported per-model media capabilities, local audio adapters where feasible, realtime duplex voice conversation, Developer ID signing/notarization validation, and broader media permission prompts.
...
- `WebSearchService` supports DuckDuckGo HTML search without an API key, SearXNG JSON search for self-hosted search, and Brave Search through the official web-search endpoint with a Keychain-backed API key read by secret ID. It applies result limits and optional domain filters before returning snippets.
...
- `SettingsView` exposes search engine, result count, SearXNG base URL, Brave API-key entry, comma-separated domain filters, page-content loading, and page text character caps; `WebSearchSettings` normalizes decoded and saved admin controls by trimming URLs/secret IDs/domains, removing duplicate domain filters, and clamping result/page-text limits to supported ranges.
...
Future work should add provider capability checks, additional API-key engines such as Tavily/SearchApi where useful, broader binary web extraction, and richer network transparency history drilldowns.
...
- `CodeExecutionService` uses macOS `Process` to run explicit user-triggered shell snippets through `/bin/zsh -lc` or Python snippets through `/usr/bin/python3 -c`. It captures stdout/stderr with `Pipe`, applies a timeout, terminates slow runs, and records failed launches as failed runs.
...
- `SettingsView` exposes the code-execution policy controls, including executable allow/deny lists, alongside feature toggles, so local command execution remains explicit and configurable.
...
Future work should add stronger sandboxing, provider tool-call routing, live PTY terminal streaming, MCP/OpenAPI tool servers, resource limits, and richer audit logs.
