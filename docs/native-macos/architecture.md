...
- `WorkspaceBackupService`: composes the current local app state into one versioned JSON backup and decodes workspace restore files. It includes persisted local data but excludes Keychain secret values by design. `AutomaticWorkspaceBackupService` writes a safety backup before destructive workspace restore/import and prunes those automatic backups to the latest ten files.
...
- `SettingsStore`: MVP local persistence for provider settings, feature toggles, and the Local Execution safety gate. Loading settings normalizes the Local Execution sandbox root and creates the directory if needed.
...
- `ChatThreadView` exposes visible and context-menu actions for copy, edit, regenerate/retry, rating, and feedback, and streaming assistant headers include a compact `Generating...` state.
...
- `ComposerView` owns focus, file-pending controls, Command+Enter send, Escape transient-state dismissal, active-generation progress, and stop-all controls. Shift+Enter remains a text-editing newline in the multi-line prompt field.
- `OpenWebUINativeApp` wires native menu shortcuts for Command+N new chat, Command+Shift+C copy selected chat as Markdown, and Command+R refresh models.
- `AppStore.sendDraftPrompt()` treats draft text as UI state and only clears it after `AppStore.send(_:)` accepts the prompt by creating the user message plus assistant branch messages. Empty, whitespace-only, no-model, unsupported-provider, and pre-stream provider failures preserve the draft and pending attachments while setting a clear composer inline message.
- `AppStore.send(_:)` preflights chat capability and model selection before creating a thread or opening provider streams. Once accepted, it appends one user message and one assistant message per selected model, clears pending attachments, and records provider/model IDs on the thread.
- Provider stream failures are stored on the affected assistant message through `ProviderErrorPresentation`, so transcript recovery stays local to the failed branch instead of only appearing as a global alert.
- Streaming updates, branch completion, outlet filters, and local pipe output find the owning chat by assistant message ID. This keeps generation consistent if the user selects another chat or sidebar surface while a response is still streaming.
- On startup, `AppStore.load()` normalizes any persisted `isStreaming=true` messages into stopped retryable assistant errors, because a JSON record from a previous app run cannot still have a live provider stream.
- Settings diagnostics expose secretless chat health: total chat count, selected chat ID/title, selected-message count, active streaming branch count, provider/model selection, last app/provider notice, and chat storage path.
...
- `AppStore.cancelCurrentSend()` and per-branch cancellation both clear `isStreaming` before the send settles, and cancellation is routed to the active provider stream task to avoid duplicate assistant messages after stop.
...
- `ToolServerMCPDiscoveryService` performs HTTP MCP discovery and stdio subprocess MCP discovery by sending JSON-RPC `initialize`, `notifications/initialized`, and `tools/list` messages with bounded stdio response timeouts, then surfaces discovered tool names, titles, descriptions, and input schemas as transient `AppToolServerTool` records. It drains stdio subprocess stderr while the session is open and cleans up the child process on close. It also supports basic HTTP and stdio MCP `tools/call` invocation with JSON-object arguments, behind Direct Tool Servers feature-toggle and `tools.execute` enforcement in `AppStore`; stdio discovery/calls also require the Local Execution safety gate, while HTTP endpoints do not.
...
- Local Execution is default-disabled. Enabling it requires accepting a Settings warning and keeping local subprocess working directories inside the configured sandbox root, which defaults to `~/OpenWebUINativeSandbox`; older broad default roots are migrated to that narrower default.
...
- Local Execution settings for the master enable switch, accepted-risk warning flag, and sandbox root.
...
- `LocalExecutionSettings` defines the master local-subprocess safety gate: disabled by default, requires an accepted risk warning before enablement, normalizes a sandbox root, and blocks requests whose working directory is outside that root.
- `CodeExecutionSettings` and `CodeExecutionPolicy` define the code-specific policy layer: allowed languages, allowed working-directory roots, executable allow/deny lists, max timeout, and output caps. Blocked requests surface a clear reason before any subprocess is launched.
- `BoundedProcessRunner` is the shared one-shot subprocess wrapper. It drains stdout and stderr concurrently, caps captured output, records truncation, applies timeouts and cooperative cancellation, terminates the child process, and escalates to `SIGKILL` after a short grace period. It is best-effort process cleanup, not OS-level containerization.
- `CodeExecutionService` runs explicit user-triggered shell snippets through `/bin/zsh -lc` or Python snippets through `/usr/bin/python3 -c` using `BoundedProcessRunner`, then records failed launches, timeouts, cancelled runs, stdout, stderr, exit codes, and timestamps as run history.
...
- `SettingsView` exposes a Local Execution warning flow with sandbox-root editing, accepted-risk toggle, master enable switch, and reset-to-safe-sandbox action. It also exposes the code-execution policy controls, including executable allow/deny lists, alongside feature toggles, while the policy model enforces persisted capture limits so local command execution remains explicit and bounded.
...
Future work should add OS-level sandbox entitlements or containerization, provider tool-call routing, live PTY terminal streaming, CPU/memory-style resource limits, and richer audit logs.
