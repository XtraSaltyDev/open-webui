...
struct JSONNoteStorageService: Sendable {
    private let rootURL: URL
    private let encoder: JSONEncoder
    private let decoder: JSONDecoder

    init(rootURL: URL = JSONNoteStorageService.defaultRootURL()) {
        self.rootURL = rootURL
        self.encoder = JSONEncoder.openWebUIEncoder
        self.decoder = JSONDecoder.openWebUIDecoder
    }

    func loadNotes() async throws -> [AppNote] {
        try ensureDirectory()
        ...
        let notes = try files.map { file in
    ...
    func save(_ note: AppNote) async throws {
        try ensureDirectory()
