...
struct JSONCalendarStorageService: Sendable {
    private let snapshotURL: URL
    private let encoder: JSONEncoder
    private let decoder: JSONDecoder

    init(snapshotURL: URL = JSONCalendarStorageService.defaultSnapshotURL()) {
        self.snapshotURL = snapshotURL
        self.encoder = JSONEncoder.openWebUIEncoder
        self.decoder = JSONDecoder.openWebUIDecoder
    }

    func loadSnapshot() async throws -> CalendarSnapshot {
    ...
    func saveSnapshot(_ snapshot: CalendarSnapshot) async throws {
