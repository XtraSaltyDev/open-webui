...
struct JSONFunctionRunStorageService: Sendable {
    private let rootURL: URL
    private let encoder: JSONEncoder
    private let decoder: JSONDecoder

    init(rootURL: URL = JSONFunctionRunStorageService.defaultRootURL()) {
        self.rootURL = rootURL
        self.encoder = JSONEncoder.openWebUIEncoder
        self.decoder = JSONDecoder.openWebUIDecoder
    }

    func loadRuns() async throws -> [AppFunctionRun] {
        try ensureDirectory()
        ...
        let runs = try files.map { file in
            ...
            return try decoder.decode(AppFunctionRun.self, from: data)
        }

        return runs.sorted { $0.startedAt > $1.startedAt }
    }

    func save(_ run: AppFunctionRun) async throws {
        try ensureDirectory()
    ...
    private static func defaultRootURL() -> URL {
        ...
        return appSupport
            ...
            .appendingPathComponent("FunctionRuns", isDirectory: true)
    }
}
