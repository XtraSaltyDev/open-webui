...
struct JSONToolStorageService: Sendable {
    private let rootURL: URL
    private let encoder: JSONEncoder
    private let decoder: JSONDecoder

    init(rootURL: URL = JSONToolStorageService.defaultRootURL()) {
        self.rootURL = rootURL
        self.encoder = JSONEncoder.openWebUIEncoder
        self.decoder = JSONDecoder.openWebUIDecoder
    }

    func loadTools() async throws -> [AppTool] {
        try ensureDirectory()
        ...
        let tools = try files.map { file in
            let data = try Data(contentsOf: file)
            return try decoder.decode(AppTool.self, from: data)
        }

        return tools.sorted { $0.updatedAt > $1.updatedAt }
    }

    func save(_ tool: AppTool) async throws {
        try ensureDirectory()
        let data = try encoder.encode(tool)
        try data.write(to: fileURL(for: tool.id), options: [.atomic])
    }
