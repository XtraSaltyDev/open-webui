...
struct JSONCodeExecutionStorageService: Sendable {
    private let rootURL: URL
    private let encoder: JSONEncoder
    private let decoder: JSONDecoder

    init(rootURL: URL = JSONCodeExecutionStorageService.defaultRootURL()) {
        self.rootURL = rootURL
        self.encoder = JSONEncoder.openWebUIEncoder
        self.decoder = JSONDecoder.openWebUIDecoder
    }

    func loadRuns() async throws -> [AppCodeExecutionRun] {
        try ensureDirectory()
        ...
        let runs = try files.map { file in
            ...
            return try decoder.decode(AppCodeExecutionRun.self, from: data)
        }

        return runs.sorted { $0.startedAt > $1.startedAt }
    }

    func save(_ run: AppCodeExecutionRun) async throws {
        try ensureDirectory()
    ...
    private static func defaultRootURL() -> URL {
        ...
        return appSupport
            ...
            .appendingPathComponent("CodeExecution", isDirectory: true)
    }
}
