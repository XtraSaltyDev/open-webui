...
struct JSONToolRunStorageService: Sendable {
    private let rootURL: URL
    private let encoder: JSONEncoder
    private let decoder: JSONDecoder

    init(rootURL: URL = JSONToolRunStorageService.defaultRootURL()) {
        self.rootURL = rootURL
        self.encoder = JSONEncoder.openWebUIEncoder
        self.decoder = JSONDecoder.openWebUIDecoder
    }

    func loadRuns() async throws -> [AppToolRun] {
        try ensureDirectory()
        ...
        let runs = try files.map { file in
            ...
            return try decoder.decode(AppToolRun.self, from: data)
        }

        return runs.sorted { $0.startedAt > $1.startedAt }
    }

    func save(_ run: AppToolRun) async throws {
        try ensureDirectory()
    ...
    private static func defaultRootURL() -> URL {
        ...
        return appSupport
            ...
            .appendingPathComponent("ToolRuns", isDirectory: true)
    }
}
